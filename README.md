...
```sh
/path/to/UT-TDD-agent-harness/scripts/ut-tdd setup --team --tl-team @org/tl --qa-team @org/qa --po-team @org/po
```

Windows では同じ操作を PowerShell wrapper で実行できます:

```powershell
C:\path\to\UT-TDD-agent-harness\scripts\ut-tdd.ps1 setup --solo
```
