...
| **self-pair (撤去済み)** | 旧: `pair_artifact: self` の doc が③ペアを兼ねる規約 (IMP-039/058)。**PLAN-RECOVERY-09 (2026-07-07) で撤去** — PO 裁定なき作り込みで欠落を pair-freeze から隠していたため。現行は L2 sub-doc が `docs/test-design/harness/L10-ux-validation-test-design.md` を直接参照し、`pair_artifact: self` は `ref-unresolved` 孤児として fail-close する。用語は git 履歴検索用に retired として残す |
