...
export const BUILTIN_GITHUB_TEMPLATES: TemplateSet = {
  ...
  "adapter/CLAUDE.md": [
    ...
    `| Claude Sonnet (\`${CLAUDE_SONNET}\`) | docs, design, UI/UX, structured review | middle (xhigh for UI/UX) |`,
    `| Claude Haiku (\`${CLAUDE_HAIKU}\`) | scouting, triage, lightweight parallel checks | high, small scoped tasks |`,
    `| GPT/Codex worker (\`${GPT_WORKER}\`) | test implementation lanes | middle |`,
    `| GPT/Codex spark (\`${GPT_SPARK}\`) | lightweight implementation lanes | high |`,
    `| GPT frontier (\`${GPT_FRONTIER}\`) | gated top-tier review/consultation | low |`,
    "",
  ...
  "adapter/.claude/settings.json": [
    ...
    '            "type": "command",',
    '            "command": "node",',
    `            "args": ${JSON.stringify([...wrapperHookArgs("claude-memory-wake")])},`,
    '            "timeout": 930,',
    '            "asyncRewake": true,',
    ...
    '            "type": "command",',
    '            "command": "node",',
    `            "args": ${JSON.stringify([...wrapperHookArgs("subagent-stop")])},`,
    '            "timeout": 5,',
  ...
  "common/harness-check.yml": [
    ...
    "  push:",
    "    branches: [main]",
    "  # universal PR trigger (PLAN-L6-82): pull_request は base 限定しない (stacked PR も発火)。",
    "  pull_request:",
    "",
    "permissions:",
    "  contents: read",
    ...
    '          node .ut-tdd/bin/ut-tdd.mjs github guard --head-ref "$HEAD_REF" --base-ref "$BASE_REF" --pr-title "$PR_TITLE" --pr-body-file .ut-tdd-pr-body.txt --commit-file .ut-tdd-commit-subjects.txt',
    "      - run: npm run typecheck",
    "      - run: npm run test",
    "      - run: node .ut-tdd/bin/ut-tdd.mjs audit quality --include-tests --limit 20",
